...
export default function Navbar() {
  ...
  return (
    <header className={scrolled ? 'fixed inset-x-0 top-0 z-50 bg-navy-800/90 shadow-2xl shadow-black/30 backdrop-blur-xl' : 'fixed inset-x-0 top-0 z-50 glass'}>
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <a href="#home" className="flex items-center gap-2 text-lg font-extrabold text-white">✈️ <span>SmartUdan<span className="text-brand-500">.</span></span></a>
        ...
        <button type="button" aria-label="Toggle menu" onClick={() => setOpen((v) => !v)} className="rounded-full border border-white/10 p-2 text-white md:hidden">
